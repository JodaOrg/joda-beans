...
class BeanGen {
    ...
    private String[] parseBeanType(int defLine) {
        ...
        for (int index = defLine; index < content.size(); index++) {
            ...
            if (matcher.matches()) {
                ...
                return new String[] {fnl, matcher.group(1), matcher.group(2), matcher.group(3), matcher.group(4), matcher.group(5), matcher.group(6)};
            }
    ...
    private String[] parseBeanSuperType(int defLine) {
        ...
        for (int index = defLine; index < content.size(); index++) {
            ...
            if (matcherExtends.matches()) {
                return new String[] {matcherExtends.group(1), matcherExtends.group(2), matcherExtends.group(3), matcherExtends.group(4)};
            }
            if (line.contains(AUTOGENERATED_START_TEXT)) {
                break;
            }
    ...
    private void generateImmutableConstructor() {
        if (data.isImmutable() && data.isImmutableConstructor() == false && data.isTypeFinal()) {
            List<PropertyGen> nonDerived = nonDerivedProperties();
    ...
    private void generateBuilderBuilder() {
        ...
        if (data.isMutable() || (data.isImmutable() && data.isImmutableConstructor() == false && data.isTypeFinal() == false)) {
