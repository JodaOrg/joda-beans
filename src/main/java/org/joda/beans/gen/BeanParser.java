...
class BeanParser {
    ...
    private int parseStartAutogen() {
        final String autogeneratedStart;
        if (this.config.isCheckstyleIgnores()) {
            autogeneratedStart = AUTOGENERATED_START + "\t//CHECKSTYLE:OFF";
        } else {
            autogeneratedStart = AUTOGENERATED_START;
        }
        for (int index = 0; index < content.size(); index++) {
            String line = content.get(index).trim();
            if (line.contains(" AUTOGENERATED START ")) {
                content.set(index, autogeneratedStart);
                return index;
            }
        }
        for (int index = content.size() - 1; index >= 0; index--) {
            String line = content.get(index).trim();
            if (line.equals("}")) {
                content.add(index, autogeneratedStart);
                return index;
            }
            if (line.length() > 0) {
                break;
    ...
    private int parseEndAutogen() {
        final String autogeneratedEnd;
        if (this.config.isCheckstyleIgnores()) {
            autogeneratedEnd = "\t//CHECKSTYLE:ON" + AUTOGENERATED_END;
        } else {
            autogeneratedEnd = AUTOGENERATED_END;
        }
        for (int index = autoStartIndex; index < content.size(); index++) {
            String line = content.get(index).trim();
            if (line.contains(" AUTOGENERATED END ")) {
                content.set(index, autogeneratedEnd);
                return index;
            }
        }
        content.add(autoStartIndex + 1, autogeneratedEnd);
